"""Config CLI + the bounding-box drawing GUI.

Pure config I/O now lives in core.config; this module keeps the interactive
pieces (the tkinter screen selector and the argparse CLI) and re-exports the
config helpers so existing imports (`from configure import load_config`) keep
working.
"""
...
from core.config import (  # re-exported for backwards compatibility
    CONFIG_FILE,
    default_config,
    load_config,
    save_config,
...
# USES A FULLSCREEN OVERLAY TO LET YOU DRAW A BOX AROUND A HEALTH BAR
...
def create_bounding_box(name=None):
    ...
    root.destroy()
    time.sleep(0.5)  # short delay to ensure the window is closed
...
def words_to_binding(word=None, binding=None):
    ...
    config = load_config()
    config.setdefault('word_bindings', {})[word.strip()] = binding.strip()
    save_config(config)
...
def main():
    parser = argparse.ArgumentParser(description="cleric config tool")
    ...
    parser.add_argument('--word-binding', nargs='?', const='', type=str, help="Specify a word to match and the binding to press")
    parser.add_argument('--auto-heal', action='store_true', help="Configure auto-heal threshold/binding")
